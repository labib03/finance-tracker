    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !isLoading && !open && onClose()}>
            <DialogContent className="sm:max-w-[400px] border-none shadow-2xl overflow-hidden p-0" showCloseButton={!isLoading}>
                ...
                <DialogFooter className="flex sm:flex-row gap-3 p-6 pt-4 bg-muted/30 border-t border-border/50 mt-4">
                    <Button 
                        variant="ghost" 
                        onClick={onClose} 
                        disabled={isLoading}
                        className="flex-1 rounded-xl font-bold hover:bg-background h-11"
                    >
                        {cancelText}
                    </Button>
                    <Button 
                        ...
                        onClick={onConfirm}
                        className={cn(
                            "flex-1 rounded-xl font-bold h-11 shadow-sm transition-all active:scale-95",
