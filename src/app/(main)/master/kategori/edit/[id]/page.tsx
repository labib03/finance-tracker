...
export default function KategoriEditPage() {
  ...
  const kategoriToEdit = kategoriList.find((k) => k.id_kategori === id);

  if (!kategoriToEdit) {
    return (
      <div className="p-8 text-center text-slate-400 min-h-screen flex flex-col items-center justify-center bg-slate-950">
        <p className="text-xl font-bold text-slate-200">Kategori tidak ditemukan</p>
        <p className="text-sm text-slate-500 mt-2">ID Kategori: {id}</p>
        <button
